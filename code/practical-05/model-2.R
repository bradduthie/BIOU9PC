...
model2 <- function(input, output, session, params){
  ...
  predation.data <- reactive({
    ...
    calc2(a = input$a, b = input$b, d = input$d, r= input$r, K = input$K, yini = c(N=input$N_init, P=input$P_init), times = params$times)
  })
  # Render population trajectories over time
  output$trajPlot <- renderPlot({
    cat(file=stderr(), "rendering plot 2 trajectories...\n") 
    trajPlot <-
      # loading_progress("Rendering plot...")
      plotstyle()
      plot2("trajectories", predation.data(), params$max.time)
  })
  ...
  output$planePlot <- renderPlot({
    ...
    planePlot <-
      # loading_progress("Rendering plot...")
      plotstyle()
      plot2("phase-plane", predation.data(), params$max.time)
  })
}