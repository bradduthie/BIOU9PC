...
model4 <- function(input, output, session, params){
  ...
  output$trajPlot <- renderPlot({
    cat(file=stderr(), "rendering plot 4 trajectories...\n") 
    trajPlot <-
      # loading_progress("Rendering plot...")
      plotstyle()
    plot4("trajectories", predation.data(), params$max.time)
  })
  # Render phase-plane diagram
  output$planePlot <- renderPlot({
    cat(file=stderr(), "rendering plot 4 phase-plane...\n") 
    planePlot <-
      # loading_progress("Rendering plot...")
      plotstyle()
    plot4("phase-plane", predation.data(), params$max.time)
  })
}