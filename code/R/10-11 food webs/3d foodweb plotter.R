...
function (foodweb, radii, cols, levels) {
    library(rgl)
    ...
    if (exists("radii")) {
        radius.code <- matrix(ncol = length(unique(t((foodweb[S +
                                                                  1, ])))))
    ...
    for (level in unique(t((foodweb[S + 1, ])))) {
        n <- length(which(net.plot[, 2] == level))
        r <- as.double(radius.code[which(rownames(radius.code) ==
                                             level)])
    ...
    plot3d(net.plot[, 1], net.plot[, 3], net.plot[, 2], type = "s",
           col = net.plot[, 4], size = 2, box = FALSE, axes = FALSE,
           xlab = "", ylab = "", zlab = "", aspect = TRUE, top = TRUE)
    for (sp in 1:S) {
        for (prey in intersect(which(foodweb[1:S, sp] == 1), which(foodweb[S + 1, ] != foodweb[S + 1, sp]))){
            lines3d(
                c(net.plot[sp, 1], net.plot[prey, 1]),
                c(net.plot[sp, 3], net.plot[prey, 3]),
                c(net.plot[sp, 2], net.plot[prey, 2]), cex = 0.1)
        }
    }
}
